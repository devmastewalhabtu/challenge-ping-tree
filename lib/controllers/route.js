...
function route (req, res) {
  body(req, res, function (err, payload) {
    ...
    TargetModel.getAllTargets(function (err, targets) {
      ...
      targets = filterTodayEligibleTargets(targets, todayKey)
      ...
      const criteria = { geoState, criteriaHour }

      const foundTarget = findCriteriaTarget(targets, criteria)
...
function filterTodayEligibleTargets (targets, todayCountKey) {
  targets.filter(
    function (target) {
      ...
      if (!target[todayCountKey]) return target

      // if today count key exists check it doesn't exceed max accept per day
      if (parseInt(target[todayCountKey]) < parseInt(target.maxAcceptsPerDay)) {
        return target
      }
    }
  )
